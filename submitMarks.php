	...
	else
	{
		...
		if($per<=35)
		{
			...
			elseif($per>=51 && $per<=70)
				...
				$sql=mysqli_query($al,"INSERT INTO marks(roll,sem,s1,s2,s3,s4,s5,total,percent,result,url) VALUES('$roll','$sem','$s1','$s2','$s3','$s4','$s5','$total','$per','$result','$url')");
				if($sql)
				{
...
<select name="roll" class="fields" style="background-color:#00000;" required>
<option value="" selected="selected" disabled="disabled">- - Select Roll/Name - -</option>
<?php
while($n=mysqli_fetch_array($m))
{
	?>
<option value="<?php echo $n['roll'];?>"><?php echo $n['roll']." ".$n['name'];?></option>
<?php } ?>
</select></td></tr>
...
<tr>
<td class="labels">Answer Sheet URL:</td><td><input type="url" name="url" class="fields" placeholder="Enter URL" size="50" required="required" /></td>
</tr>
